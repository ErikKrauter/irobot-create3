...
State DriveStraightBehavior::execute(const Data & data)
{
    ...
    const bool ir_ = true;
    bool hazards_detected = is_front_hazard_active(data.hazards, data.ir_readings, ir_);
    // Pointing towards dock or found hazard
    if (driving_towards_dock || hazards_detected) {
        RCLCPP_INFO(m_logger, "Stop driving straight: traveled %f/%f: hazards %ld ir_signals %ld dock %d",
            traveled_distance, m_config.max_distance,
            data.hazards.detections.size(), data.ir_readings.readings.size(), driving_towards_dock);
