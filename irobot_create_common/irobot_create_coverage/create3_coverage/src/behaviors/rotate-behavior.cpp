...
{
    const bool ir_ = true;
    if (!is_front_hazard_active(data.hazards, data.ir_readings, ir_) && !m_reflex_behavior) {
        return State::SUCCESS;
    }
    ...
    if (m_config.robot_has_reflexes) {
        ...
        const bool ir_ = false;
        if (!is_front_hazard_active(data.hazards, data.ir_readings, ir_)) {
            return State::SUCCESS;
        } else {
            return State::RUNNING;
        }
