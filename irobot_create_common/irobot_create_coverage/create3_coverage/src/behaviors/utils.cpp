...
bool is_front_hazard_active(const irobot_create_msgs::msg::HazardDetectionVector& hazards, const irobot_create_msgs::msg::IrIntensityVector& ir_vec, const bool ir_)
{
    using irobot_create_msgs::msg::IrIntensity;

    if (ir_ == true){
        for (const IrIntensity& ir_measurement : ir_vec.readings) {
            if (ir_measurement.header.frame_id == "ir_intensity_front_left" ||
                ir_measurement.header.frame_id == "ir_intensity_front_right" ||
                ir_measurement.header.frame_id == "ir_intensity_front_center_right" ||
                ir_measurement.header.frame_id == "ir_intensity_front_center_left"){

                    if (ir_measurement.value > 15) {

                        std::cout << "IR SENSOR DETECTS OBSTACLE" << std::endl;
                        std::cout << "frame id: "  << ir_measurement.header.frame_id << std::endl;
                        std::cout << "value: "  << ir_measurement.value << std::endl;
                        return true;
                    }
            }
    ...
    for (const HazardDetection& detection : hazards.detections) {
        if (detection.type != HazardDetection::BACKUP_LIMIT) {
            std::cout << "BUMPER SENSOR DETECTS OBSTACLE" << std::endl;
            return true;
        }
    }
