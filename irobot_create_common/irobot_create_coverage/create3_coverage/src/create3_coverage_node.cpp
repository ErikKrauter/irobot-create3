...
: rclcpp::Node("create3_coverage")
{
    ...
    m_ir_subscription = this->create_subscription<IrMsg>(
        "ir_intensity",
        rclcpp::SensorDataQoS(),
        std::bind(&Create3CoverageNode::ir_intensity_callback, this, _1));
...
void Create3CoverageNode::execute(const std::shared_ptr<GoalHandleCoverage> goal_handle)
{
    ...
    do {
        ...
        Behavior::Data data;
        {
            ...
            data.ir_readings = m_last_ir_readings;
...
bool Create3CoverageNode::reflexes_setup()
{
    ...
    const std::vector<std::string> param_names = {
        ...
        "reflexes.REFLEX_OBJECT_PROXIMITY",
        "reflexes_enabled"
    };
...
bool Create3CoverageNode::ready_to_start()
{
    ...
    */
    ...
    if (m_dock_subscription->get_publisher_count() == 0)
    {
        RCLCPP_WARN(this->get_logger(), "m_dock_subscription haven't discovered their publishers yet");
        return false;
    }

    if (m_hazards_subscription->get_publisher_count() == 0)
    {
        RCLCPP_WARN(this->get_logger(), "m_hazards_subscription haven't discovered their publishers yet");
        return false;
    }

    if (m_ir_opcode_subscription->get_publisher_count() == 0)
    {
        RCLCPP_WARN(this->get_logger(), "m_ir_opcode_subscription haven't discovered their publishers yet");
        return false;
    }

    if (m_odom_subscription->get_publisher_count() == 0)
    {
        RCLCPP_WARN(this->get_logger(), "m_odom_subscription haven't discovered their publishers yet");
        return false;
    }

    if (m_kidnap_subscription->get_publisher_count() == 0)
    {
        RCLCPP_WARN(this->get_logger(), "m_kidnap_subscription haven't discovered their publishers yet");
        return false;
    }
...
void Create3CoverageNode::ir_intensity_callback(IrMsg::ConstSharedPtr msg)
{
    std::lock_guard<std::mutex> guard(m_mutex);
    m_last_ir_readings = *msg;
}
